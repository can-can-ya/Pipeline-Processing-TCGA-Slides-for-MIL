...
class WholeSlideImage(object):
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
        ...
        # Thresholding
        if use_otsu:
            # Determind the threshold automatically using OTSU alg.
            _, img_otsu = cv2.threshold(img_med, 0, sthresh_up, cv2.THRESH_OTSU+cv2.THRESH_BINARY)
        else:
            # 255 if V > sthresh else 0
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        if patch_level >= len(self.level_downsamples):
            print("Patch level is {}, but highest level is {}, skip".format(patch_level, len(self.level_downsamples)-1))
            return {}, {}
        patch_downsample = (int(self.level_downsamples[patch_level][0]), int(self.level_downsamples[patch_level][1]))
        ref_patch_size = (patch_size*patch_downsample[0], patch_size*patch_downsample[1])
